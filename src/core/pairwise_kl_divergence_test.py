...
def loss_with_kl_div(P, xp, Q, xq, margin):
    ...
    KLPQ = tf.reduce_sum(tf.multiply(P ,tf.log(tf.div(P, Q))))
    KLQP = tf.reduce_sum(tf.multiply(Q ,tf.log(tf.div(Q , P))))
    lossPQ = tf.add(tf.multiply(Is, KLPQ) , tf.multiply(Ids ,tf.maximum(0., tf.subtract(margin,KLPQ))))
    lossQP = tf.add(tf.multiply(Is, KLQP) , tf.multiply(Ids , tf.maximum(0., tf.subtract(margin,KLQP))))
