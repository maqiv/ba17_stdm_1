...
def outerLoop(x, tf_l, predictions, labels, margin):
    ...
    y = tf.constant(0)
    res = tf.while_loop(innerLoop_cond, innerLoop, [y, x, tf_l, predictions, labels, margin], swap_memory=True, parallel_iterations = 20,  name='innerloop')
...
def pairwise_kl_divergence(labels, predictions):
    ...
    #loss = tf.Variable(0.)
    #tf_l = tf.Variable(0. , name='loss')
    sum_loss = tf.while_loop(outerLoop_condition, outerLoop, [x, tf_l, predictions, labels, margin], swap_memory=True, parallel_iterations = 20, name='outerloop')
    n = tf.constant(100.)
