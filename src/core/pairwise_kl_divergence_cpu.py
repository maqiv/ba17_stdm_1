...
def outerLoop(x, tf_l, predictions, labels, margin):
    with tf.device('/cpu:0'):
        ...
        y = tf.constant(0)
        res = tf.while_loop(innerLoop_cond, innerLoop, [y, x, tf_l, predictions, labels, margin], parallel_iterations = 10 , name='innerloop')
...
def pairwise_kl_divergence(labels, predictions):
    with tf.device('/cpu:0'):
        x = tf.constant(0)
        sum_loss = tf.while_loop(outerLoop_condition, outerLoop, [x, tf_l, predictions, labels, margin], parallel_iterations = 10 , name='outerloop')
