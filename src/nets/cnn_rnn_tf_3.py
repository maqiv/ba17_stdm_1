...
class cnn_rnn_tf_3(object):
    ...
    def create_net(self):
        ...
        with tf.name_scope('Dense'):
            dense = tf.layers.dense(inputs=gru_out, units=cnn_rnn_tf_3.stngs['total_speakers']*10, activation=tf.nn.relu)

        cnn_rnn_tf_3.logger.info("Creating softmax layer")
        with tf.name_scope('Softmax'):
            gru_soft_out = tf.layers.dense(inputs=dense, units=cnn_rnn_tf_3.stngs['total_speakers'], activation=tf.nn.softmax)
        ...
        return optimizer, gru_soft_out, dense, cross_entropy, accuracy, x_input, out_labels
    ...
    def run_network(self):
        ...
        optimizer, gru_soft_out, dense, cross_entropy, accuracy, x_input, out_labels = self.create_net()
        ...
        cnn_rnn_tf_3.logger.info("Loading train data for GRU evaluation")
        with open(os.path.join(cnn_rnn_tf_3.stngs['gru']['data_path'], cnn_rnn_tf_3.stngs['gru']['train_data_file']), 'rb') as f:
            (train_raw_x_data, raw_y_data, train_speaker_names) = pickle.load(f)
            train_x_data, train_y_data = dg.generate_test_data(train_raw_x_data, raw_y_data, segment_size=cnn_rnn_tf_3.stngs['segment_size'])

        train_net_output = dense.eval(feed_dict={x_input: np.reshape(train_x_data, [train_x_data.shape[0], train_x_data.shape[2], train_x_data.shape[3], train_x_data.shape[1]])}, session=sess)

        cnn_rnn_tf_3.logger.info("Loading test data for GRU evaluation")
        with open(os.path.join(cnn_rnn_tf_3.stngs['gru']['data_path'], cnn_rnn_tf_3.stngs['gru']['test_data_file']), 'rb') as f:
            (test_raw_x_data, raw_y_data, test_speaker_names) = pickle.load(f)
            test_x_data, test_y_data = dg.generate_test_data(test_raw_x_data, raw_y_data, segment_size=cnn_rnn_tf_3.stngs['segment_size'])

        test_net_output = dense.eval(feed_dict={x_input: np.reshape(test_x_data, [test_x_data.shape[0], test_x_data.shape[2], test_x_data.shape[3], test_x_data.shape[1]])}, session=sess)


        # Write output file for clustering
        cnn_rnn_tf_3.logger.info("Write outcome to pickle files for clustering")
        with open(os.path.join(cnn_rnn_tf_3.stngs['cluster_output_path'], (cnn_rnn_tf_3.stngs['cluster_output_train_file'] + self.date_time + '.pickle')), 'wb') as f:
            pickle.dump((train_net_output, train_y_data, train_speaker_names), f, -1)

        with open(os.path.join(cnn_rnn_tf_3.stngs['cluster_output_path'], (cnn_rnn_tf_3.stngs['cluster_output_test_file'] + self.date_time + '.pickle')), 'wb') as f:
